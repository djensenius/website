---
title: FoundSounds
year: 2015
summary: An international collaborative art project disguised as a social network (2010–2015).
tags:
  - collaborative
  - social
status: complete
links:
  - label: More information
    url: https://foundsounds.me
  - label: FoundSounds API source
    url: https://github.com/FoundSounds/FoundSounds-API
  - label: FoundSounds iOS source
    url: https://github.com/FoundSounds/FoundSounds-iOS
  - label: FoundSounds web source
    url: https://github.com/FoundSounds/FoundSounds-Web
---

An international collaborative art project disguised as a social network, running
from 2010 to 2015. FoundSounds invited people around the world to record, share,
and recombine the sounds of their everyday environments.
